...
/* XXX '#if 0' - YIKES. Apparently this function is unused now so remove it? */
#if 0
/*
...
{
	/* addis r4,0,(insn)@h */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_ADDIS | ___PPC_RT(4) |
			  ((val >> 16) & 0xffff)),
			  patch_err);
	addr++;

	/* ori r4,r4,(insn)@l */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_ORI | ___PPC_RA(4) |
 ...
 */
static int patch_imm64_load_insns(unsigned long val, int reg, kprobe_opcode_t *addr)
{
	/* lis reg,(op)@highest */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_ADDIS | ___PPC_RT(reg) |
			  ((val >> 48) & 0xffff)),
			  patch_err);
	addr++;

	/* ori reg,reg,(op)@higher */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_ORI | ___PPC_RA(reg) |
			  ___PPC_RS(reg) | ((val >> 32) & 0xffff)),
			  patch_err);
	addr++;

	/* rldicr reg,reg,32,31 */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_RLDICR | ___PPC_RA(reg) |
			  ___PPC_RS(reg) | __PPC_SH64(32) | __PPC_ME64(31)),
			  patch_err);
	addr++;

	/* oris reg,reg,(op)@h */
	PATCH_INSN_OR_GOTO(addr, ppc_inst(PPC_INST_ORIS | ___PPC_RA(reg) |
...
int arch_prepare_optimized_kprobe(struct optimized_kprobe *op, struct kprobe *p)
{
	...
	for (i = 0; i < size; i++)
		PATCH_INSN_OR_GOTO((struct ppc_inst *)(buff + i),
	 ...
	 */
	if (patch_imm64_load_insns((unsigned long)op, 3, buff + TMPL_OP_IDX))
		goto patch_err;
	...
	PATCH_INSN_OR_GOTO((struct ppc_inst *)(buff + TMPL_CALL_HDLR_IDX),
				branch_op_callback, patch_err);

	PATCH_INSN_OR_GOTO((struct ppc_inst *)(buff + TMPL_EMULATE_IDX),
				branch_emulate_step, patch_err);

	/*
	 * 3. load instruction to be emulated into relevant register, and
	 */
	temp = ppc_inst_read((struct ppc_inst *)p->ainsn.insn);
	if (patch_imm64_load_insns(ppc_inst_as_u64(temp), 4, buff + TMPL_INSN_IDX))
		goto patch_err;

	/*
	 * 4. branch back from trampoline
	 */
	create_branch(&temp, (struct ppc_inst *)(buff + TMPL_RET_IDX),
			(unsigned long)nip, 0);
	PATCH_INSN_OR_GOTO((struct ppc_inst *)(buff + TMPL_RET_IDX), temp,
				patch_err);
...
void arch_optimize_kprobes(struct list_head *oplist)
{
	...
	list_for_each_entry_safe(op, tmp, oplist, list) {
		...
		rc = patch_instruction((struct ppc_inst *)op->kp.addr, instr);
		if (rc)
